from fastapi import APIRouter, Depends, HTTPException
from sqlalchemy.orm import Session
from app.schemas import parent_schema
from app.crud import parent_crud
from app.database import get_db
from app.core.role_checker import RoleChecker, get_current_user
from app.models.user import User, UserRole


router = APIRouter(prefix="/parents", tags=["Parents"])

# RBAC role groups
admin_roles = RoleChecker([UserRole.super_admin, UserRole.school_admin])


@router.post("/", response_model=parent_schema.ParentOut, dependencies=[Depends(admin_roles)])
def create_parent(parent: parent_schema.ParentCreate, db: Session = Depends(get_db)):
    """Admins create a parent"""
    result = parent_crud.create_parent(db, parent)
    if isinstance(result, dict) and "error" in result:
        raise HTTPException(status_code=400, detail=result["error"])
    return result


@router.get("/{parent_id}", response_model=parent_schema.ParentOut)
def read_parent(parent_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    """Parent can read own record, admins can read any"""
    parent = parent_crud.get_parent(db, parent_id)
    if not parent:
        raise HTTPException(status_code=404, detail="Parent not found")

    # RBAC logic
    if current_user.role in ["super_admin", "school_admin"]:
        return parent
    if current_user.role == "parent" and current_user.user_id == parent.user_id:
        return parent

    raise HTTPException(status_code=403, detail="Not authorized to view this parent")


@router.get("/", response_model=list[parent_schema.ParentOut], dependencies=[Depends(admin_roles)])
def read_parents(skip: int = 0, limit: int = 10, db: Session = Depends(get_db)):
    """Only admins can list all parents"""
    return parent_crud.get_parents(db, skip, limit)


@router.put("/{parent_id}", response_model=parent_schema.ParentOut, dependencies=[Depends(admin_roles)])
def update_parent(parent_id: int, update_data: parent_schema.ParentCreate, db: Session = Depends(get_db)):
    """Admins update parent"""
    result = parent_crud.update_parent(db, parent_id, update_data)
    if not result:
        raise HTTPException(status_code=404, detail="Parent not found")
    if isinstance(result, dict) and "error" in result:
        raise HTTPException(status_code=400, detail=result["error"])
    return result


@router.delete("/{parent_id}", response_model=parent_schema.ParentOut, dependencies=[Depends(admin_roles)])
def delete_parent(parent_id: int, db: Session = Depends(get_db)):
    """Admins delete parent"""
    parent = parent_crud.delete_parent(db, parent_id)
    if not parent:
        raise HTTPException(status_code=404, detail="Parent not found")
    return parent
